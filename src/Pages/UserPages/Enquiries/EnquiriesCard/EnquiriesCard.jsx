...
const EnquiriesCard = ({ enquiryDatas }) => {
  ...
  useEffect(() => {
    ...
    enquiryDatas?.userid ? (
      instance.get(`/api/profile/${enquiryDatas?.userid}`)
        .then((response) => {
    ...
    instance.get(`/api/contactsviewed/${user?.userid}`)
      .then((response) => {
  ...
  const handleUnlock = () => {
    ...
    instance.get(`/api/subscriptionusers/${user?.userid}?plantype=${plantype}`)
      .then((res1) => {
        ...
        if (res1.data) {
          ...
          instance.post(`/api/contactsviewed`, contactViewedPayload)
            .then(((res2) => {
              console.log("contactsviewed response", res2)
              // console.log("plan id", res1?.data[0]?.subscriptionusers_id)

              if (res2.statusText == "OK") {
                instance.put(`/api/subscriptionusers/update/${res1?.data[0]?.subscriptionusers_id}`)
                  .then((res3) => {
        ...
        <div className="flex gap-10 lg:flex-row md:flex-col flex-col p-4">
          ...
          <div className="flex flex-col gap-5  px-8 py-4">
            <div className="mx-auto">
              <img src={`http://16.170.140.185:3000/${profileData?.profileimagepath}`} className="h-[200px] " />
            </div>
